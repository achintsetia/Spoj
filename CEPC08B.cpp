...
void event( int i, int N)
{
  if( h[i] == -1) return;

  h[i] = -1;
  if( i>0 && h[i-1] == -1 && i<N-1 && h[i+1] == -1){ ans--; return;}
  if( i==0 && h[i+1] == -1) { ans--; return;}
  if( i==N-1 && h[i-1] == -1) { ans--; return;}

  if( i > 0 && h[i-1] == -1 ) return;
  if( i < N-1 && h[i+1] == -1 ) return;
  if( i == 0 || i == N-1) return;
  ans++;
}
