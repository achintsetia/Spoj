...
vector< pair<int, int> > actArray;

int solve( vector< pair<int,int> > actArray)
{
  sort( actArray.begin(), actArray.end());

  int f = actArray[0].first;
  int count = 1;

  int sz = actArray.size();
  for( int i=1; i<sz; i++)
    if( actArray[i].second >= f)
    {
      count++;
      f = actArray[i].first;
    }
...
int main( int argc, char** argv)
{
  ...
  while( T--)
  {
    ...
    for( int i=0; i<N; i++)
    {
      ...
      actArray.push_back(make_pair(end, start));
    }

    output( solve( actArray), "\n");
  }
  return 0;
}
