...
int main( int argc, char** argv)
{
  ...
  while( T--)
  {
    int NG = input.readInt();
    int NM = input.readInt();

    vector<int> ngVec;
    vector<int> nmVec;

    for( int i=0; i<NG; i++)
      ngVec.push_back(input.readInt());
    for( int i=0; i<NM; i++)
      nmVec.push_back(input.readInt());

    sort(ngVec.begin(), ngVec.end());
    sort(nmVec.begin(), nmVec.end());

    int ngCount = 0, nmCount = 0;
    while( ngCount < ngVec.size() && nmCount < nmVec.size())
    {
      if( nmVec[nmCount] == ngVec[ngCount])
        nmCount++;

      ngVec[ngCount] < nmVec[nmCount] ? ngCount++ : nmCount++;
    }

    if( ngCount < ngVec.size()) output("Godzilla\n");
    else output("MechaGodzilla\n");
  }
  return 0;
}
