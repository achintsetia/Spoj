...
long long int inversions(int arr[], int N, int start, int end)
{
  ...
  long long int ans = 0; int mid = (start+end)/2;
  ans += inversions(arr, N, start, mid);
  ans += inversions(arr, N, mid+1, end);
  ...
  while(i<=mid || j <= end)
  {
    if( i<=mid && j<=end)
    {
      if(arr[i] <= arr[j]) Aux[count++] = arr[i++];
      else
      {
        ans += mid-i+1;;
        Aux[count++] = arr[j++];
    ...
    {
      if( i > mid) Aux[count++] = arr[j++];
      else if( j > end) { Aux[count++] = arr[i++];}
    }
  }

  for( int i=start, k=0; i<=end; i++) arr[i] = Aux[k++];

  //cout << "S: " << start << " " << end << " INV: " << ans << endl;
  return ans;
}
