...
int solve(int N)
{
  ...
  for( int i=1; i<N; i++)
  {
    ...
    for( int j=0; j<i; j++)
    {
      if( blocks[j].ff < blocks[i].ff &&
          blocks[j].fs < blocks[i].fs &&
          blocks[j].s > maxHeight)
      {
        maxHeight = blocks[j].s;
      }
    }
    blocks[i].s += maxHeight;
  }

  int maxHeight = blocks[0].s;
  for( int i=1; i<N; i++)
    maxHeight = max(maxHeight, blocks[i].s);
...
int main( int argc, char** argv)
{
  ...
  while(N > 0)
  {
    ...
    for( int i=0; i<N; i++)
    {
      ...
      piii a = make_pair(make_pair(min(w,d), max(w,d)), h);
      temp = h; h = d; d = temp;
      piii b = make_pair(make_pair(min(w,d), max(w,d)), h);
      temp = h; h = w; w = temp;
      piii c = make_pair(make_pair(min(w,d), max(w,d)), h);
