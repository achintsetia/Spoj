...
int ans( int arr[], int N)
{
  ...
  for( int i=1; i<N-1; i++)
  {
    int MaxA = 0, MaxB = 0, MaxAI = 0, MaxBI = 0;
    for( int j=0; j<i; j++) if( LCSF[j] > MaxA) {MaxA = LCSF[j]; MaxAI = j;}
    for( int j=i; j<N; j++) if( LCSB[j] > MaxB) {MaxB = LCSB[j]; MaxBI = j;}

    if( (arr[MaxAI] > arr[MaxBI] || arr[MaxBI] > arr[MaxAI]) && MaxLen < MaxA + MaxB)
      MaxLen = MaxA + MaxB;
  }
  return MaxLen;
}
